...
def get_todays_news():
	return scrape_article("http://www.letemps.ch/economie/2016/06/16/bns-ne-croit"
                   "-brexit-s-y-prepare")
...
def scrape_article(source_url):
	...
	try:
		reading_time = article_info.find('p',class_='reading-time').text.strip()
	except:
		reading_time = "unknown"
	...
	author = "unknown"
	try:
		author = article_info.find('a',class_='article-author').text.strip()
	except:
		author = "unknown"
